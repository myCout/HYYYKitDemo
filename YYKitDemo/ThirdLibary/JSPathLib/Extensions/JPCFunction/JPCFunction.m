...
#define JP_DEFINE_TYPE_ENCODE_CASE(_type) \
    ...
    for (NSInteger i = 0; i < typeArr.count; i++) {
        NSString *ltypeStr = trim([typeArr objectAtIndex:i]);
        NSString *encode = _typeEncodeDict[ltypeStr];
        if (!encode) {
            NSString *argClassName = trim([ltypeStr stringByReplacingOccurrencesOfString:@"*" withString:@""]);
