...
{
//    __weak HYNetworkManager *weakSelf;
}
@end
...
{
    self.resposeBlock = completionBlock;
...
//    session.responseSerializer=[AFHTTPResponseSerializer serializer];
    ...
    {
        [[HYAFHTTPSessionManager sharedClient] POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
- (void)returnDataWithRequestData:(NSData *)requestData
{
    ...
    if ([myResult isKindOfClass:[NSDictionary  class]]) {
        ...
        //        //根据返回的接口内容来变 、key 要根据服务器返回的进行对应的修改
        ...
        NSLog(@"返回Json\n%@\n",response);
        //把data层剥掉
        NSDictionary *dict = [response objectForKey:@"data"];//key 要根据服务器返回的进行对应的修改
...
-(void)showSuccess:(id)response
{
    if (self.resposeBlock) {
        self.resposeBlock(nil,response);
    }
