            ...
            if ([UIFont jk_deviceHasFullSupportForFontCollections]) {
                ...
                if (fontDescriptors) {
                    ...
                    for (NSDictionary *fontDescriptor in fontDescriptors) {
                        ...
                        if (fontPSName) {
                            if ([UIFont fontWithName:fontPSName size:kSizePlaceholder]) {
                                NSLog(@"Warning with font registration: Font '%@' already registered",fontPSName);
                            }
                    ...
                    if ([fontPSNames count] > 0) {
                        ...
                        NSLog(@"Warning with font registration: All fonts in '%@' are already registered", fontURL);
                    }
                    
                } else { // CTFontManagerCreateFontDescriptorsFromURL fail
                    NSLog(@"Error with font registration: File '%@' is not a Font", fontURL);
                    fontPSNames = nil;
                }
                ...
                if (fontData) {
                    ...
                    NSLog(@"Error with font registration: URL '%@' cannot be read with error: %@", fontURL, error);
                    fontPSNames = nil;
                }
...
+ (UIFont *) jk_customFontWithURL:(NSURL *)fontURL size:(CGFloat)size {
    ...
    if (![singleFontValidExtensions containsObject:[fontURL pathExtension]]) {
        NSLog(@"Only ttf or otf files are supported by this method");
        return nil;
    }
    ...
    if (fontPSNames == nil) {
        NSLog(@"Invalid Font URL: %@", fontURL);
        return nil;
    }
    if ([fontPSNames count] != 1) {
        NSLog(@"Font collections not supported by this method");
        return nil;
    }
