...
+(void)initialize{
    ...
    //创建队列
    FMDatabaseQueue *queue =[FMDatabaseQueue databaseQueueWithPath:[dbPath stringByAppendingString:dbName]];
    ...
    if(queue==nil){
        ...
        [self executeQuery:sql queryResBlock:^(FMResultSet *set) {
            while ([set next]) {
                [dict setObject: [NSMutableDictionary dictionary]  forKey:[set stringForColumn:@"name"]];
            }
        }];
        [[DBMangeShare sharedDBMange].tableDict setObject:dict forKey:TableClassName];
    }
